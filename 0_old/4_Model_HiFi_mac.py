...
def build_model(input_shape, config):
    ...
    # define model
    model = keras.Sequential(name='model_1')
    ...
    # Add Conv1D layers
    for _ in range(config['N_Conv1d_layers']):
        model.add(keras.layers.Conv1D(filters=config['filter_size'], kernel_size=config['kernel_size'], padding='same'))
        model.add(keras.layers.Activation(config['activation_func']))

    # Add the final Conv1D layer without activation layer
    model.add(keras.layers.Conv1D(config['model_output_channels'], kernel_size=config['kernel_size'], padding='same', name='output_layer'))
